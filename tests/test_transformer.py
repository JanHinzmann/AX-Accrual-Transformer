...
def test_default_csv_has_utf8_bom_crlf_and_exact_headers(tmp_path):
    ...
    assert "\r\n" in text
    assert rows[0]["Buchungstext"].startswith("8.2026 RUECK Österreichische Apotheker")
...
def test_comma_delimiter_quotes_commas_and_round_trips(tmp_path):
    ...
    assert headers == list(AX_HEADERS)
    assert rows[0]["Buchungstext"] == "8.2026 RUECK Vendor, Inc. Consulting fee , V-007"
    assert '"8.2026 RUECK Vendor, Inc. Consulting fee , V-007"' in text
