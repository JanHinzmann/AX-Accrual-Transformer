...
def _validate_ax_rows(rows: list[_SourceRow]) -> tuple[dict[int, Decimal], list[RowIssue]]:
    ...
    for row in rows:
        ...
        description_parts = (
            row.values["Month"],
            row.values["Vendor name"],
            row.values["Posting description"],
            row.values["Vendor ID"],
        )
        if all(_is_blank(value) for value in description_parts):
            issues.append(
                RowIssue(
                    row.company_id,
                    row.row_number,
                    "Buchungstext",
                    "Month, vendor, posting description, and Vendor ID are all empty.",
                )
            )
